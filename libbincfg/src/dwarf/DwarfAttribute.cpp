...
DwarfAttr::DwarfAttr(Dwarf_Attribute d_attr)
    : m_type(0),
      ...
      m_isValid(false)
{
    //Log::log() << "reading generic attr" << ELogLevel::LOG_DEBUG;
    do {
        if (dwarf_whatattr(d_attr, &m_type, nullptr) != DW_DLV_OK) {
            Log::log() << "...whatattr failed" << ELogLevel::LOG_DEBUG;
            break;
        }

        m_form.reset(Dwarf::createDwarfForm(d_attr));
        if (m_form.get() == nullptr) {
            Log::log() << "...createDwarfForm failed" << ELogLevel::LOG_DEBUG;
            break;
        }
        if (!m_form->isValid()) {
            Log::log() << "...form invalid" << ELogLevel::LOG_DEBUG;
            break;
        }
...
{
#if 0
    bool haveLo = false, haveHi = false, haveName = false;
    for (AttrSet::iterator it = attrSet.begin(); it != attrSet.end(); ++it) {
        Dwarf_Half tag = (*it)->getType();
        if (tag == DW_AT_name) haveName = true;
        if (tag == DW_AT_low_pc) haveLo = true;
        if (tag == DW_AT_high_pc) haveHi = true;
        //std::string strtag = Dwarf::getTagType(tag);
        //Log::log() << "Validate subprogram: attr " << strtag
        //           << "=" << (*it)->getValueAsString() << ELogLevel::LOG_DEBUG;
    }
    return haveName && haveLo && haveHi;
#else
    // some subs are almost empty, and just used as abstract origin for callsites. need to keep.
    return true;
#endif
}
...
Dwarf::getAttributes(Dwarf_Debug d_debug, Dwarf_Die d_inDie, bool& status)
{
    ...
    do {
        ...
        if (!isValidDie) {
            Dwarf_Half d_tagValue;
            dwarf_tag(d_inDie, &d_tagValue, nullptr);
            Log::log() << "Could not validate attributes for DIE @0x" << std::hex
                       << d_dieOffset << " "
                       << Dwarf::getTagType(d_tagValue)
                       << ELogLevel::LOG_WARNING;
