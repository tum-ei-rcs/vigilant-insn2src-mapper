...
{TIC(0); 
  ...
  /* Init loop */
  for(i = 0; TIC(31), i <= n; i++)
    {
      w = 0.0;              /* data to fill in cells */
      for(j = 0; TIC(42), j <= n; j++)
        {
          a[i][j] = (i + 1) + (j + 1);
  ...
  TIC(102)/* TODO: missing time from ludcmp */, chkerr = ludcmp(nmax,n);
}
...
{TIC(1034); 
  ...
  for(i = 0; TIC(284), i < n; i++)
    {
      /* if(fabs(a[i][i]) <= eps) return(1); */
      for(j = i+1; TIC(818), j <= n; j++) /* triangular loop vs. i */
        {
          w = a[j][i];
          if(i != 0)            /* sub-loop is conditional, done
                                   all iterations except first of the
                                   OUTER loop */
            for(k = 0; TIC(105), k < i; k++)
              w -= a[j][k] * a[k][i];
          a[j][i] = w / a[i][i];
        }
      for(j = i+1; TIC(68), j <= n; j++) /* triangular loop vs. i */
        {
          w = a[i+1][j];
          for(k = 0; TIC(105), k <= i; k++) /* triangular loop vs. i */
            w -= a[i+1][k] * a[k][j];
          a[i+1][j] = w;
        }
    }
  y[0] = b[0];
  for(i = 1; TIC(60), i <= n; i++)       /* iterates n times */
    {
      w = b[i];
      for(j = 0; TIC(103), j < i; j++)    /* triangular sub loop */
        w -= a[i][j] * y[j];
      y[i] = w;
    }
  x[n] = y[n] / a[n][n];
  for(i = n-1; TIC(827), i >= 0; i--)     /* iterates n times */
    {
      w = y[i];
      for(j = i+1; TIC(107), j <= n; j++) /* triangular sub loop */
        w -= a[i][j] * x[j];
      x[i] = w / a[i][i] ;
    }
  TIC(49); return(0);
