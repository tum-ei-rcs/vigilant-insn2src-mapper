...
void main()
{TIC(0); 
   TIC(22)/* TODO: missing time from Test */, InitSeed();
/* ***UPPSALA WCET***:
   no printing please! */
#ifndef UPPSALAWCET
...
{TIC(0); 
   ...
   TIC(59)/* TODO: missing time from Initialize */, Initialize(A);
   Initialize(B);
...
{TIC(0); 
   ...
   for (OuterIndex = 0; TIC(7), OuterIndex < UPPERLIMIT; OuterIndex++)
      for (InnerIndex = 0; TIC(16)/* TODO: missing time from RandomInteger */, InnerIndex < UPPERLIMIT; InnerIndex++)
...
{TIC(0); 
   ...
   for (Outer = 0; TIC(17), Outer < UPPERLIMIT; Outer++)
      for (Inner = 0; TIC(16), Inner < UPPERLIMIT; Inner++)
      {
         Res [Outer][Inner] = 0;
         for (Index = 0; TIC(36), Index < UPPERLIMIT; Index++)
            Res[Outer][Inner]  +=
