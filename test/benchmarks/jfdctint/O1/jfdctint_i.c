...
{TIC(0); 
  ...
  dataptr = data;
  for (ctr = DCTSIZE-1; TIC(476), ctr >= 0; ctr--) {
  ...
  dataptr = data;
  for (ctr = DCTSIZE-1; TIC(440), ctr >= 0; ctr--) {
...
{TIC(0); 
  ...
  seed = 1;
  for (i = 0; TIC(769), i < 64; i++) {
    seed = ((seed * 133) + 81) % 65535;
    data[i] = seed;
  }

  TIC(63)/* TODO: missing time from jpeg_fdct_islow */, jpeg_fdct_islow();
}
