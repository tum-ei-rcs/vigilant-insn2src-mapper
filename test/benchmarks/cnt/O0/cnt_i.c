...
int main (void)
{TIC(0); 
   TIC(34)/* TODO: missing time from InitSeed */, InitSeed();
...
{TIC(0); 
   TIC(1451)/* TODO: missing time from Sum */; long StartTime, StopTime;
   float TotalTime;
...
{TIC(0); 
   ...
   for (OuterIndex = 0; TIC(5), OuterIndex < MAXSIZE; TIC(7), OuterIndex++) //100 + 1
      for (InnerIndex = 0; TIC(4), InnerIndex < MAXSIZE; TIC(37)/* TODO: missing time from RandomInteger */, InnerIndex++) //100 + 1
...
{TIC(0); 
  ...
  for (Outer = 0; TIC(14), Outer < MAXSIZE; Outer++) //Maxsize = 100
    for (Inner = 0; TIC(36), Inner < MAXSIZE; Inner++)
#ifdef WORSTCASE
