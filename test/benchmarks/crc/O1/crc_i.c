...
{TIC(101); 
  ...
  if (!init) {
    TIC(27), init=1;
    for (j=0;TIC(63)/* TODO: missing time from icrc1 */, j<=255;j++) {
      icrctb[j]=icrc1(j << 8,(uchar)0);
      rchr[j]=(uchar)(it[j & 0xF] << 4 | it[j >> 4]);
    }
  }
  if (jinit >= 0) TIC(11), cword=((uchar) jinit) | (((uchar) jinit) << 8);
  else if (TIC(19), jrev < 0)
    TIC(25), cword=rchr[HIBYTE(cword)] | rchr[LOBYTE(cword)] << 8;
#ifdef DEBUG
  printf("len = %d\n", len);
#endif
  for (j=1;TIC(28), j<=len;j++) {
    if (jrev < 0) {
      TIC(9), tmp1 = rchr[lin[j]]^ HIBYTE(cword);
    }
    else {
      TIC(6), tmp1 = lin[j]^ HIBYTE(cword);
    }
...
{TIC(0); 
  ...
  TIC(62)/* TODO: missing time from icrc */; unsigned short i1,i2;
  unsigned long n;
